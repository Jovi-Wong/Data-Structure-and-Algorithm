...
template<typename type>
int RBT<type>::RB_INSERT(type num)//creat a new node whose key is num
{
...
template<typename type>
RBT<type>::RBT(Node<type>* a)//constructor 1
{
	root->key = a->key;
	root->p = a->p;
	root->left = a->left;
...
template<typename type>
int RBT<type>::RIGHT_ROTATE(Node<type>* x)//same as LEFT_ROTATE
{
	Node<type>* y = x->left;
	x->left = y->right;
	if (y->right != nil)
...
template<typename type>
int RBT<type>::RB_INSERT_FIXUP(Node<type>* z)// adjust structure to fit in rule of red black tree
{
	while (z->p->color == RED)
	{
		if (z->p == z->p->p->left)
...
int RBT<type>::RB_INSERT(Node<type>* z)
{
    ...
    z->right = nil;
    z->color = RED;
    RB_INSERT_FIXUP(z);//change structure
    return 0;
};
...
template<typename type>
Node<type>* RBT<type>::TREE_MINIMUM(Node<type>* x)//return the minimum of subtree
{
	Node<type>* y = x;
